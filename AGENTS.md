...
- Request bodies are validated by hand. Unknown fields are rejected, values are never coerced from another type, and `dryRun` must be a real boolean, so a mistyped dry run cannot become a real operation. IDs passed to `docker` or `virsh` cannot start with `-`. Mutating CA and plugin OpenClaw tools declare `additionalProperties: false` and re-check their parameters before making any HTTP request.
...
- Mutations run bounded commands with timeouts and verify real state afterward, such as `docker inspect`, `virsh domstate`, `mdcmd status` or the plugin registration, not just the exit code of a helper. Responses carry `verified`; a transition that is legitimately asynchronous, such as a VM shutdown, reports `verified: false` instead of failing. Follow this pattern for new mutations.
