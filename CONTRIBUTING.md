...
- **Releases.** The maintainer renames `### Unreleased` to the version, sets the same version in `packages/cli/package.json`, writes `release-notes/<version>.md` following `release-notes/README.md`, then pushes a `v<version>` tag. The release workflow publishes that file as the release notes with a standard install and downloads section, and stops before building if the notes file or the changelog section is missing. It attaches the plugin package and `unraidclaw-cli-<version>.tar.gz`, built by `packages/cli/scripts/package.sh`, with checksums. The maintainer then publishes the CLI to npm with `pnpm publish` from `packages/cli`, which runs its build and tests first.
